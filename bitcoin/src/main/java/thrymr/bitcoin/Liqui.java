...
public class Liqui {
	...
	public static String Url = "https://liqui.io/";
	public static void Liqui_data(WebDriver driver) throws Exception {
		Properties prop = new Properties();
		InputStream input = new FileInputStream("./inputdata.properties");
		prop.load(input);
		File file = new File(prop.getProperty("excelpath"));
		driver.get(Url);
		WaitForElementVisible(driver, By.xpath("//li[contains(text(),'USDT')]"));
		driver.findElement(By.xpath("//li[contains(text(),'USDT')]")).click();
		Thread.sleep(3000);
		WebElement Table = driver.findElement(By.xpath("//table[@class='table table-hover markets-table']//tbody"));
		...
		for (int i = 0; i < rows.size(); i++) {
			try {
				...
				Exceldata.writeXLSXFile(Pair, Last_Price.trim(), file.toString(), "Liqui");
			}
